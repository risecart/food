...
export const OrderExchange = () => {
  ...
  // server
  const { isPending, mutateAsync } = trackingService.useTracking();
  const [data, setData] = useState<OrderFull[]>([]);
  ...
  const getTraking = async (data: Type) => {
    mutateAsync({
      ...
      let s = res.filter((el) => isExchange(el));
      if (s.length) setData(s);
      else {
        toast.error(t("no_exchange"));
      }
    });
  };
  ...
  return (
    <form onSubmit={handleSubmit(getTraking)}>
      <div className="flex justify-center items-center mt-6 p-3 flex-col gap-5">
        <RiExchangeLine className=" w-28 h-28 text-primary opacity-80" />
